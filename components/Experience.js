      ...
      <div className="w-full md:w-2/3 px-8 pl-12">
        {experiences &&
          experiences.map((item, index) => (
            <div
              className={`flex justify-between ${index !== 0 && "mt-6"}`}
              key={index}
            >
              <div>
                <h5 className="font-normal text-text text-base mb-1">
                  {item.company}
                </h5>
                <p className="font-normal text-text font-mono text-sm block md:hidden mt-1 mb-2">
                  {item.duration}
                </p>
                <p className="small-text">{item.position}</p>
              </div>
              <p className="font-normal text-textLight font-mono text-sm hidden md:block">
                {item.duration}
              </p>
            </div>
          ))}
